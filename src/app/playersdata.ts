...
export class PlayersData {
  ...
  public players: Array<{name: string, isInGame: boolean, games: number, upperTotal: number, total: number,
  ...
  add(name, isInGame) {
	  if (this.exists(name) == -1) {
			this.players.push({name:name, isInGame: isInGame, games: 0, upperTotal: 0, total: 0, 
